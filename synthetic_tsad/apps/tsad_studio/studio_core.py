...
FIELD_LABELS = {
    ...
    "scale_events_by_sequence_length": "Scale Events By Length",
    "sequence_length_reference": "Length Reference",
...
FIELD_DESCRIPTIONS = {
    ...
    "distribution": "Sampling law used inside the configured sequence-length range.",
    "decay_scale": "Decay scale for truncated-exponential sequence-length sampling. Smaller values bias harder toward short sequences.",
    ...
    "scale_events_by_sequence_length": "If true, sampled event count is scaled by (sequence_length / sequence_length_reference).",
    "sequence_length_reference": "Reference length used by event-count scaling.",
...
def _build_select_options() -> dict[str, list[str]]:
    return {
        "sequence_length.distribution": ["uniform", "truncated_exponential"],
...
SCALAR_BOUNDS: dict[str, tuple[float, float, str]] = {
    ...
    "seed": (1, 999_999, "int"),
    "sequence_length.decay_scale": (1.0, 10_000.0, "float"),
